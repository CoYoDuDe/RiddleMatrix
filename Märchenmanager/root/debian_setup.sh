...
echo "🚀 Starte vollständige Debian-Einrichtung für Märchen Manager..."

## 📦 Pakete installieren
echo "📦 Installiere Pakete..."
apt update
apt install -y python3 python3-pip dnsmasq hostapd rfkill curl wget xserver-xorg xinit x11-xserver-utils firefox-esr firmware-linux-nonfree firmware-iwlwifi firmware-realtek firmware-misc-nonfree libffi-dev libssl-dev build-essential linux-image-cloud-amd64

## 🐍 Python-Abhängigkeiten
echo "🐍 Installiere Python-Abhängigkeiten..."
pip3 install --no-warn-script-location flask beautifulsoup4 requests

## 📁 Projektverzeichnis & Dateien
mkdir -p /usr/local/bin /usr/local/etc /opt/webkiosk /home/kioskuser/.X.d
...
def find_config_path():
    for d in ["/mnt/sd" + chr(i) + "2" for i in range(97, 123)]:
        if os.path.isdir(os.path.join(d, "boxen_config")):
            return os.path.join(d, "boxen_config", "boxen_config.json")
    config_path = "/mnt/sdb2/boxen_config/boxen_config.json"
...
## 🛠️ XFCE deaktivieren
echo "🛠️ Deaktiviere XFCE Display-Manager..."
systemctl disable lightdm || true

## 🛠️ Sudo für poweroff
echo "🛠️ Erlaube poweroff für kioskuser ohne Passwort..."
echo "kioskuser ALL=(ALL) NOPASSWD: /sbin/poweroff" > /etc/sudoers.d/kioskuser
...
systemctl disable bluetooth networking cron avahi-daemon cups ModemManager NetworkManager
...
echo "   sudo reboot"
echo "⚠️ Hinweis: Prüfe Boot-Zeit mit 'systemd-analyze'. Erstelle ein Image mit 'dd' für weitere Sticks."